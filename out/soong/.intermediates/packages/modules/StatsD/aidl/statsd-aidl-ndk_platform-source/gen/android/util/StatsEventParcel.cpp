...
namespace android {
namespace util {
const char* StatsEventParcel::descriptor = "android.util.StatsEventParcel";

binder_status_t StatsEventParcel::readFromParcel(const AParcel* parcel) {
  ...
  _aidl_ret_status = ::ndk::AParcel_readVector(parcel, &buffer);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  AParcel_setDataPosition(parcel, _aidl_start_pos + _aidl_parcelable_size);
  return _aidl_ret_status;
}
binder_status_t StatsEventParcel::writeToParcel(AParcel* parcel) const {
  binder_status_t _aidl_ret_status;
  size_t _aidl_start_pos = AParcel_getDataPosition(parcel);
  _aidl_ret_status = AParcel_writeInt32(parcel, 0);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  _aidl_ret_status = ::ndk::AParcel_writeVector(parcel, buffer);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
