...
namespace android {
namespace net {
const char* ResolverHostsParcel::descriptor = "android.net.ResolverHostsParcel";

binder_status_t ResolverHostsParcel::readFromParcel(const AParcel* parcel) {
  ...
  _aidl_ret_status = ::ndk::AParcel_readString(parcel, &ipAddr);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
...
binder_status_t ResolverHostsParcel::writeToParcel(AParcel* parcel) const {
  binder_status_t _aidl_ret_status;
  size_t _aidl_start_pos = AParcel_getDataPosition(parcel);
  _aidl_ret_status = AParcel_writeInt32(parcel, 0);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  _aidl_ret_status = ::ndk::AParcel_writeString(parcel, ipAddr);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
