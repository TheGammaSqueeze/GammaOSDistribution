...
namespace resolv {
namespace aidl {
const char* DnsHealthEventParcel::descriptor = "android.net.resolv.aidl.DnsHealthEventParcel";

binder_status_t DnsHealthEventParcel::readFromParcel(const AParcel* parcel) {
  ...
  _aidl_ret_status = AParcel_readInt32(parcel, &healthResult);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
  ...
  _aidl_ret_status = ::ndk::AParcel_readVector(parcel, &successRttMicros);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  AParcel_setDataPosition(parcel, _aidl_start_pos + _aidl_parcelable_size);
  return _aidl_ret_status;
}
binder_status_t DnsHealthEventParcel::writeToParcel(AParcel* parcel) const {
  ...
  _aidl_ret_status = AParcel_writeInt32(parcel, healthResult);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  _aidl_ret_status = ::ndk::AParcel_writeVector(parcel, successRttMicros);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
