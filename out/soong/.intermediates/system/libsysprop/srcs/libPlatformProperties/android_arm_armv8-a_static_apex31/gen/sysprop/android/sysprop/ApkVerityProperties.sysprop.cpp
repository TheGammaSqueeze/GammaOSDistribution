...
#include <android/sysprop/ApkVerityProperties.sysprop.h>
...
using namespace android::sysprop::ApkVerityProperties;
...
std::optional<std::int32_t> apk_verity_mode() {
    return GetProp<std::optional<std::int32_t>>("ro.apk_verity.mode");
}

bool apk_verity_mode(const std::optional<std::int32_t>& value) {
    return SetProp("ro.apk_verity.mode", FormatValue(value).c_str());
}
