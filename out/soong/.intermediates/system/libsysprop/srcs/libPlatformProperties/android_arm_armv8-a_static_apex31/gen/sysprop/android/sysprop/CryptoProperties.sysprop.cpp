...
#include <android/sysprop/CryptoProperties.sysprop.h>
...
using namespace android::sysprop::CryptoProperties;

template <typename T> T DoParse(const char* str);

constexpr const std::pair<const char*, type_values> type_list[] = {
    {"block", type_values::BLOCK},
    {"file", type_values::FILE},
    {"none", type_values::NONE},
};

template <>
std::optional<type_values> DoParse(const char* str) {
    for (auto [name, val] : type_list) {
...
std::string FormatValue(std::optional<type_values> value) {
    if (!value) return "";
    for (auto [name, val] : type_list) {
        if (val == *value) {
            return name;
    ...
    LOG_ALWAYS_FATAL("Invalid value %d for property ro.crypto.type", static_cast<std::int32_t>(*value));
    __builtin_unreachable();
}

constexpr const std::pair<const char*, state_values> state_list[] = {
    {"encrypted", state_values::ENCRYPTED},
    {"unencrypted", state_values::UNENCRYPTED},
    {"unsupported", state_values::UNSUPPORTED},
};

template <>
std::optional<state_values> DoParse(const char* str) {
    for (auto [name, val] : state_list) {
        if (strcmp(str, name) == 0) {
            return val;
        }
    }
...
std::optional<type_values> type() {
    return GetProp<std::optional<type_values>>("ro.crypto.type");
}

bool type(const std::optional<type_values>& value) {
    return SetProp("ro.crypto.type", FormatValue(value).c_str());
}

std::optional<state_values> state() {
    return GetProp<std::optional<state_values>>("ro.crypto.state");
}
