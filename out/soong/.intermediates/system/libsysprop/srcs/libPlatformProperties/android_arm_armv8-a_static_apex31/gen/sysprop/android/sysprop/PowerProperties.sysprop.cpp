...
#include <android/sysprop/PowerProperties.sysprop.h>
...
using namespace android::sysprop::PowerProperties;
...
std::optional<std::int32_t> fixed_performance_scale_factor() {
    return GetProp<std::optional<std::int32_t>>("ro.power.fixed_performance_scale_factor");
}

std::optional<bool> battery_input_suspended() {
    return GetProp<std::optional<bool>>("power.battery_input.suspended");
}

bool battery_input_suspended(const std::optional<bool>& value) {
    return SetProp("power.battery_input.suspended", FormatValue(value).c_str());
}
