...
binder_status_t Authorization::readFromParcel(const AParcel* parcel) {
  ...
  _aidl_ret_status = ::ndk::AParcel_readParcelable(parcel, &keyParameter);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  AParcel_setDataPosition(parcel, _aidl_start_pos + _aidl_parcelable_size);
  return _aidl_ret_status;
}
binder_status_t Authorization::writeToParcel(AParcel* parcel) const {
  ...
  _aidl_ret_status = ::ndk::AParcel_writeParcelable(parcel, keyParameter);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
