// Generated by protoc-gen-cppstream. DO NOT MODIFY.
// source: frameworks/base/core/proto/android/view/insetssourcecontrol.proto

#ifndef ANDROID_FRAMEWORKS_BASE_CORE_PROTO_ANDROID_VIEW_INSETSSOURCECONTROL_PROTO_STREAM_H
#define ANDROID_FRAMEWORKS_BASE_CORE_PROTO_ANDROID_VIEW_INSETSSOURCECONTROL_PROTO_STREAM_H

namespace android {
namespace view {

// message InsetsSourceControlProto
namespace InsetsSourceControlProto {
    // optional string type = 1;
    const uint64_t TYPE = 0x0000010900000001LL;

    // optional .android.graphics.PointProto position = 2;
    const uint64_t POSITION = 0x0000010b00000002LL;

    // optional .android.view.SurfaceControlProto leash = 3;
    const uint64_t LEASH = 0x0000010b00000003LL;

    static const int _FIELD_COUNT = 3;
    static const char* _FIELD_NAMES[3] = {
        "type",
        "position",
        "leash",
    };
    static const uint64_t _FIELD_IDS[3] = {
        TYPE,
        POSITION,
        LEASH,
    };

} //InsetsSourceControlProto

} // view
} // android

#endif // ANDROID_FRAMEWORKS_BASE_CORE_PROTO_ANDROID_VIEW_INSETSSOURCECONTROL_PROTO_STREAM_H
