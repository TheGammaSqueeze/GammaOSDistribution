// Generated by protoc-gen-cppstream. DO NOT MODIFY.
// source: frameworks/base/core/proto/android/view/insetscontroller.proto

#ifndef ANDROID_FRAMEWORKS_BASE_CORE_PROTO_ANDROID_VIEW_INSETSCONTROLLER_PROTO_STREAM_H
#define ANDROID_FRAMEWORKS_BASE_CORE_PROTO_ANDROID_VIEW_INSETSCONTROLLER_PROTO_STREAM_H

namespace android {
namespace view {

// message InsetsControllerProto
namespace InsetsControllerProto {
    // optional .android.view.InsetsStateProto state = 1;
    const uint64_t STATE = 0x0000010b00000001LL;

    // repeated .android.view.InsetsAnimationControlImplProto control = 2;
    const uint64_t CONTROL = 0x0000020b00000002LL;

    static const int _FIELD_COUNT = 2;
    static const char* _FIELD_NAMES[2] = {
        "state",
        "control",
    };
    static const uint64_t _FIELD_IDS[2] = {
        STATE,
        CONTROL,
    };

} //InsetsControllerProto

} // view
} // android

#endif // ANDROID_FRAMEWORKS_BASE_CORE_PROTO_ANDROID_VIEW_INSETSCONTROLLER_PROTO_STREAM_H
