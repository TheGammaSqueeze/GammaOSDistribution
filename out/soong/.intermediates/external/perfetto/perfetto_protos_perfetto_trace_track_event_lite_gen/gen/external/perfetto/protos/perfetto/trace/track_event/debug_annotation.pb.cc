    ...
    switch (tag >> 3) {
      ...
      case 11:
        if (PROTOBUF_PREDICT_TRUE(static_cast<::PROTOBUF_NAMESPACE_ID::uint8>(tag) == 90)) {
          ...
          do {
            ptr += 1;
            ptr = ctx->ParseMessage(add_dict_entries(), ptr);
            CHK_(ptr);
            if (!ctx->DataAvailable(ptr)) break;
...
void DebugAnnotation::SerializeWithCachedSizes(
  ...
  // repeated .perfetto.protos.DebugAnnotation dict_entries = 11;
  for (unsigned int i = 0,
      n = static_cast<unsigned int>(this->dict_entries_size()); i < n; i++) {
    ::PROTOBUF_NAMESPACE_ID::internal::WireFormatLite::WriteMessage(
      11,
      this->dict_entries(static_cast<int>(i)),
      output);
  }
...
// @@protoc_insertion_point(message_byte_size_start:perfetto.protos.DebugAnnotation)
  ...
  // repeated .perfetto.protos.DebugAnnotation dict_entries = 11;
  {
    unsigned int count = static_cast<unsigned int>(this->dict_entries_size());
    total_size += 1UL * count;
    for (unsigned int i = 0; i < count; i++) {
      total_size +=
        ::PROTOBUF_NAMESPACE_ID::internal::WireFormatLite::MessageSize(
...
void DebugAnnotation::InternalSwap(DebugAnnotation* other) {
  ...
  _internal_metadata_.Swap(&other->_internal_metadata_);
  swap(_has_bits_[0], other->_has_bits_[0]);
  CastToBase(&dict_entries_)->InternalSwap(CastToBase(&other->dict_entries_));
  CastToBase(&array_values_)->InternalSwap(CastToBase(&other->array_values_));
