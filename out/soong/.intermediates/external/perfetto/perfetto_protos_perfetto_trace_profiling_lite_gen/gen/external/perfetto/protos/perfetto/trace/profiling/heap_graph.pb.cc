...
HeapGraphObject::HeapGraphObject(const HeapGraphObject& from)
  ...
  ::memcpy(&type_id_, &from.type_id_,
    ...
    reinterpret_cast<char*>(&type_id_)) + sizeof(reference_field_id_base_));
...
#define CHK_(x) if (PROTOBUF_PREDICT_FALSE(!(x))) goto failure
  ...
  while (!ctx->Done(&ptr)) {
    ...
    switch (tag >> 3) {
      ...
      case 7:
        if (PROTOBUF_PREDICT_TRUE(static_cast<::PROTOBUF_NAMESPACE_ID::uint8>(tag) == 56)) {
          set_id_delta(::PROTOBUF_NAMESPACE_ID::internal::ReadVarint(&ptr));
          CHK_(ptr);
        } else goto handle_unusual;
        continue;
      default: {
...
void HeapGraphObject::SerializeWithCachedSizes(
  ...
  // optional uint64 id_delta = 7;
  if (has_id_delta()) {
    ::PROTOBUF_NAMESPACE_ID::internal::WireFormatLite::WriteUInt64(7, this->id_delta(), output);
  }
