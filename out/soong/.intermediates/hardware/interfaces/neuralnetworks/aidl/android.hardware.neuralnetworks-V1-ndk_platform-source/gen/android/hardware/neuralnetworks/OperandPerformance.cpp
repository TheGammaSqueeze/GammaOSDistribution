...
const char* OperandPerformance::descriptor = "android.hardware.neuralnetworks.OperandPerformance";

binder_status_t OperandPerformance::readFromParcel(const AParcel* parcel) {
  ...
  _aidl_ret_status = ::ndk::AParcel_readParcelable(parcel, &info);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  AParcel_setDataPosition(parcel, _aidl_start_pos + _aidl_parcelable_size);
  return _aidl_ret_status;
}
binder_status_t OperandPerformance::writeToParcel(AParcel* parcel) const {
  ...
  _aidl_ret_status = ::ndk::AParcel_writeParcelable(parcel, info);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
