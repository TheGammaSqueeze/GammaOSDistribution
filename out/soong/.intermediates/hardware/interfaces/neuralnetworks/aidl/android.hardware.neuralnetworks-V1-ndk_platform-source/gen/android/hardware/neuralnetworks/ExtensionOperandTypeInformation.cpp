...
const char* ExtensionOperandTypeInformation::descriptor = "android.hardware.neuralnetworks.ExtensionOperandTypeInformation";

binder_status_t ExtensionOperandTypeInformation::readFromParcel(const AParcel* parcel) {
  ...
  _aidl_ret_status = AParcel_readChar(parcel, &type);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
  ...
  _aidl_ret_status = AParcel_readBool(parcel, &isTensor);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
  ...
  _aidl_ret_status = AParcel_readInt32(parcel, &byteSize);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  AParcel_setDataPosition(parcel, _aidl_start_pos + _aidl_parcelable_size);
  return _aidl_ret_status;
}
binder_status_t ExtensionOperandTypeInformation::writeToParcel(AParcel* parcel) const {
  binder_status_t _aidl_ret_status;
  size_t _aidl_start_pos = AParcel_getDataPosition(parcel);
  _aidl_ret_status = AParcel_writeInt32(parcel, 0);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  _aidl_ret_status = AParcel_writeChar(parcel, type);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  _aidl_ret_status = AParcel_writeBool(parcel, isTensor);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  _aidl_ret_status = AParcel_writeInt32(parcel, byteSize);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
