...
binder_status_t StateResidency::readFromParcel(const AParcel* parcel) {
  ...
  _aidl_ret_status = AParcel_readInt64(parcel, &totalTimeInStateMs);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
  ...
  _aidl_ret_status = AParcel_readInt64(parcel, &totalStateEntryCount);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
  ...
  _aidl_ret_status = AParcel_readInt64(parcel, &lastEntryTimestampMs);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  AParcel_setDataPosition(parcel, _aidl_start_pos + _aidl_parcelable_size);
  return _aidl_ret_status;
}
binder_status_t StateResidency::writeToParcel(AParcel* parcel) const {
  ...
  _aidl_ret_status = AParcel_writeInt64(parcel, totalTimeInStateMs);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  _aidl_ret_status = AParcel_writeInt64(parcel, totalStateEntryCount);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  _aidl_ret_status = AParcel_writeInt64(parcel, lastEntryTimestampMs);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
