...
binder_status_t WorkDuration::readFromParcel(const AParcel* parcel) {
  ...
  _aidl_ret_status = AParcel_readInt64(parcel, &timeStampNanos);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
  ...
  _aidl_ret_status = AParcel_readInt64(parcel, &durationNanos);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  AParcel_setDataPosition(parcel, _aidl_start_pos + _aidl_parcelable_size);
  return _aidl_ret_status;
}
binder_status_t WorkDuration::writeToParcel(AParcel* parcel) const {
  binder_status_t _aidl_ret_status;
  size_t _aidl_start_pos = AParcel_getDataPosition(parcel);
  _aidl_ret_status = AParcel_writeInt32(parcel, 0);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  _aidl_ret_status = AParcel_writeInt64(parcel, timeStampNanos);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  _aidl_ret_status = AParcel_writeInt64(parcel, durationNanos);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
