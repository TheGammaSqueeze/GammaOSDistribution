...
binder_status_t RpcHardwareInfo::readFromParcel(const AParcel* parcel) {
  ...
  _aidl_ret_status = ::ndk::AParcel_readString(parcel, &rpcAuthorName);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
  ...
  _aidl_ret_status = AParcel_readInt32(parcel, &supportedEekCurve);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  AParcel_setDataPosition(parcel, _aidl_start_pos + _aidl_parcelable_size);
  return _aidl_ret_status;
}
binder_status_t RpcHardwareInfo::writeToParcel(AParcel* parcel) const {
  ...
  _aidl_ret_status = ::ndk::AParcel_writeString(parcel, rpcAuthorName);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  _aidl_ret_status = AParcel_writeInt32(parcel, supportedEekCurve);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
