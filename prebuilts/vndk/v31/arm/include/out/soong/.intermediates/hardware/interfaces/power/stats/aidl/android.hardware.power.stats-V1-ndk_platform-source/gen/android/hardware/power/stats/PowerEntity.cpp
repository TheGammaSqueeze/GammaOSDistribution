...
binder_status_t PowerEntity::readFromParcel(const AParcel* parcel) {
  ...
  _aidl_ret_status = ::ndk::AParcel_readVector(parcel, &states);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  AParcel_setDataPosition(parcel, _aidl_start_pos + _aidl_parcelable_size);
  return _aidl_ret_status;
}
binder_status_t PowerEntity::writeToParcel(AParcel* parcel) const {
  ...
  _aidl_ret_status = ::ndk::AParcel_writeVector(parcel, states);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
