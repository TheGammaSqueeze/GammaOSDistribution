...
binder_status_t ElapsedRealtime::readFromParcel(const AParcel* parcel) {
  ...
  _aidl_ret_status = AParcel_readInt64(parcel, &timestampNs);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
  ...
  _aidl_ret_status = AParcel_readDouble(parcel, &timeUncertaintyNs);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  AParcel_setDataPosition(parcel, _aidl_start_pos + _aidl_parcelable_size);
  return _aidl_ret_status;
}
binder_status_t ElapsedRealtime::writeToParcel(AParcel* parcel) const {
  ...
  _aidl_ret_status = AParcel_writeInt64(parcel, timestampNs);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;

  _aidl_ret_status = AParcel_writeDouble(parcel, timeUncertaintyNs);
  if (_aidl_ret_status != STATUS_OK) return _aidl_ret_status;
